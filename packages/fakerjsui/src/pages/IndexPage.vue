<template>
  <q-dialog v-model="beastModeDialog" persistent>
    <q-card style="width: 700px;">
      ...
      <q-separator/>
      <q-card-section>
        <!-- Parameter Fields for Beast Mode -->
        <div v-if="selectedAction.params?.length" class="q-mb-md">
          ...
          <ParameterField
            ...
            :key="param.name"
            :param="param"
            v-model="beastModeParams[param.name]"
          />
  ...
  <ParameterDialog
    ...
    @generate="handleParameterGenerate"
  />
                  ...
                  <div style="height: 250px; overflow: scroll">
                    <template v-for="(action, actionIndex) in group" :key="actionIndex">
                      ...
                      <q-item
                        clickable
                        v-ripple
                        @click="handleMethodClick(action)"
                      >
                        <q-item-section>
                          <q-item-label class="text-weight-regular">
                            {{ action.name }}
                            <q-icon
                              v-if="action.params?.length"
                              name="tune"
                              size="xs"
                              color="grey-6"
                              class="q-ml-xs"
...
export default {
  ...
  setup () {
    ...
    async function invokeFakerFn ({ name, fakerFn, options }) {
      const result = await fakerFn({ options });
    ...
    const parameterMethod = ref(null);

    function handleMethodClick (action) {
      if (action.params?.length > 0) {
        parameterMethod.value = action;
        parameterDialog.value = true;
      } else {
        invokeFakerFn(action);
      }
    }

    async function handleParameterGenerate ({ method, options }) {
      await invokeFakerFn({
        name: method.name,
        fakerFn: method.fakerFn,
        options,
      });
    }
    ...
    const beastModeParams = ref({});

    function openBeastMode (action) {
      beastModeDialog.value = true;
      selectedAction.value = action;
      // Initialize beast mode params with defaults
      beastModeParams.value = {};
      if (action.params?.length) {
        action.params.forEach(param => {
          beastModeParams.value[param.name] = param.default;
        });
      }
    }
    ...
    function buildBeastModeOptions () {
      if (!selectedAction.value.params?.length) {
        return undefined;
      }
      const options = {};
      let hasValues = false;
      selectedAction.value.params.forEach(param => {
        const value = beastModeParams.value[param.name];
    ...
    async function bulkResult () {
      ...
      const options = buildBeastModeOptions();
      let i = 0;
      const finalResult = [];
      do {
        const result = await fakerFn({ options });
